...
pthread_cond_t pro_cond, cos_cond;
...
void *producer(void *arg)
{
    ...
    while (alive) {
        ...
        if (counter == BUFSIZE)  //buffer가 꽉찼음을 소비자에게 알림
        pthread_cond_signal(&cos_cond);
...
void *consumer(void *arg)
{
    ...
    while (alive) {
        ...
        for (int a = 0 ; a < 5 ; ++a)
            if (counter == 0)
             pthread_cond_wait(&cos_cond,&mutex);
        pthread_mutex_unlock(&mutex);
        ...
        pthread_mutex_lock(&mutex);
        if (counter == 0)        // buffer가 비어있음을 생산자에게 알림
...
int main(void)
{
    ...
    pthread_cond_init(&cos_cond, NULL);
    /*
