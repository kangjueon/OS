...
int main(void)
{
   ...
   gettimeofday(&start, NULL);
    ...
    #pragma omp parallel for num_threads(GROUP)  //GROUP개만큼의 스레드를 생성하여 병렬처리
    ...
    gettimeofday(&start, NULL);
    printf("병렬 계산 (OpenMP)...\n");
    count = 0;  // openMP로 세기위한 초기화

    omp_set_num_threads(omp_get_max_threads());  // 스레드를 코어의 개수만큼 생성하기 위함.
