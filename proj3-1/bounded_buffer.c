...
void *producer(void *arg)
{
    ...
    while (alive) {
        ...
        */
        ...
        while(atomic_load(&counter) == BUFSIZE)
            ;
        /*
         * 새로운 아이템을 생산하여 버퍼에 넣고 관련 변수를 갱신한다.
         */
        
         while(!atomic_compare_exchange_weak(&lock, &expected, true))  //buffer
            expected = false;


        item = atomic_fetch_add(&next_item,1);
        atomic_store(&buffer[in] , item);  //buffer에 item을 집어 넣는다.
        in = atomic_fetch_add(&in, 1) % BUFSIZE;
...
void *consumer(void *arg)
{
    ...
    while (alive) {
         ...
         while(atomic_load(&counter) == 0)  // 버퍼가 비어있으면 대기
            ;
        ...
        atomic_store(&item, buffer[out]);
        atomic_store(&out, atomic_fetch_add(&out, 1) % BUFSIZE);
        atomic_fetch_sub(&counter,1);
        
        
        expected = true;
        atomic_compare_exchange_weak(&lock, &expected, false);
        /*
