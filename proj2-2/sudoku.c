 ...
 * 
 *  2023.04.02 강주언 2017013663
 *  2023.04.13 mutex 공유자원 문제를 해결을 했다.
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
void *check_rows(void *arg)
{
    ...
    for (int k = 0 ; k < 9 ; k++){  //valid[0] 초기화
        valid[0][k] = true;
    }    
    for (int i = 0; i < 9 ; i++){
        
        bool found[9] = {false};   //각 행마다 숫자가 있는지 검증.
      
        for (int j = 0; j < 9; j++) {
            
            int n = sudoku[i][j]; 
            
            if (n < 1 || n > 9 || found[n-1]) {  //found[n-1]이 true가 나왔다는 뜻은 중복된 수가 있다는 뜻이다.
               valid[0][i] = false;
            }
            found[n-1] = true;
        }        
...
void *check_columns(void *arg)
{
   ...
   //int tid = *(int*)arg;
    pthread_mutex_lock(&mutex); 
    //memset(valid[1], true, sizeof(valid[1]));
    for (int k = 0 ; k < 9 ; k++){
        valid[1][k] = true;
    }    
    for (int j = 0; j < 9 ; j++){
        bool found[9] = {false};   //각 열마다 숫자가 있는지 검증.
...
{
    /* 서브 그리드의 행과 열을 탐색할떄, k/3 *3 과 같은 연산을 진행하는데
     * k = 0,1,2일때는 나눗셈의 특성으로 인해 0이 된다. 
     */
    ...
    for (int k = 0; k < 9; k++) {  // 서브그리드를 탐색
        
        bool found[9] = {false};   // 각 서브그리드마다 숫자가 있는지 검증.
       
        
        for (int i =(k / 3) * 3; i < (k / 3) * 3 + 3; i++) {  // 서브그리드의 행을 탐색
            for (int j = k % 3 * 3; j < k % 3 * 3 + 3; j++) {  // 서브그리드의 열을 탐색
               

                int n = sudoku[i][j];
                
                if (n < 1 || n > 9 || found[n-1]) {  // found[n-1]이 true면 중복된 수가 있다는 것을 의미한다.
...
{
    int i, j; 
    pthread_t tid[11];   //11개의 thread생성
    int tid_args[11];
    int rc, t;
    ...
    tid_args[0] = 0;
    rc = pthread_create(&tid[0], NULL, check_rows, (void *)&tid_args[0]);
    if (rc) {
        printf("ERROR: 스레드가 성공적으로 생성되지 않음\n");
        exit(-1);
    }
   ...
   // pthread_mutex_lock(&mutex_valid);
    tid_args[1] = 1;
    rc = pthread_create(&tid[1], NULL, check_columns, (void *)&tid_args[1]);
    if (rc) {
        printf("ERROR: 스레드가 성공적으로 생성되지 않음\n");
        exit(-1);
    }
   // pthread_mutex_unlock(&mutex_valid);
    ...
    for (t = 2; t < 11; t++) {
        ...
        tid_args[t] = t;
        rc = pthread_create(&tid[t], NULL, check_subgrid, (void *)&tid_args[t]); // 스레드가 생성되면 0을 반환. 아니면 오류
        if (rc) {   
            printf("ERROR: 스레드가 성공적으로 생성되지 않음\n");
            exit(-1);
        }
