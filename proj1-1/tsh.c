...
static void cmdexec(char *cmd)
{
    ...
    char *p, *q;                /* 명령어를 파싱하기 위한 변수 */  
     ...
     //char cwd[1024];            /*open() 경로를 위한 선언. getcwd*/
    int fd[2]={-1,-1};
    /*
	...
	*/
            ...
            if (*q) argv[argc++] = q;
             ...
             if (argv[argc]) {   // < 다음에 있는 인자를 파일로 받아들임.
        	fd[0] = open(argv[argc], O_RDONLY);   // 파일 디스크립터에 표준 입력을 저장
        	if (fd[0] == -1) {
           	  perror("open");
           	  exit(1);
        	} 
       		dup2(fd[0], 0);   // redirect standard input to fdin
        	close(fd[0]);
        	argc++; 
 	     }         
        } 
    	   ...
    	   if (argv[argc]){
            fd[1] = open(argv[argc], O_WRONLY|O_CREAT|O_TRUNC, 0644);   //표준 출력. 파일이 없다면 파일을 생성한
            if (fd[1] == -1) {
                perror("open");
                exit(1);
            	}
              dup2(fd[1], 1);   // redirect standard input to fdout
              close(fd[1]);
              argc++; 
      	    }         
        }
